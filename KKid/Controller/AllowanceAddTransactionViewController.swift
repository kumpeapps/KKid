...
//    MARK: submitForm
    func submitForm(){
        ...
        KKidClient.addAllowanceTransaction(userID: Int(selectedUser!.userID), amount: amount, description: reason.value, transactionType: transactionType) { (success, error) in
            if success{
                dispatchOnMain {
